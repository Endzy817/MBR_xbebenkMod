	...
	Switch $RoutineType
		Case "FCR0" ; FIrstCheckRoutine
			...
			Else
				...
				'CollectAchievements', 'ForgeClanCapitalGold', 'CollectCCGold', 'AutoUpgradeCC', 'SaleMagicItem', 'CollectFreeMagicItems']
				_ArrayShuffle($aRndFuncList)
				For $Index In $aRndFuncList
					If Not $g_bRunState Then Return
					_RunFunction($Index)
		...
		Case "SA3" ; switch
			Local $aRndFuncList = ['CollectFreeMagicItems', 'DonateCC,Train', 'UpgradeLow', 'BuilderBase', 'UpgradeWall', 'UpgradeHeroes', 'DonateCC,Train']
			For $Index In $aRndFuncList
				If Not $g_bRunState Then Return
				_RunFunction($Index)
				If _Sleep(50) Then Return
