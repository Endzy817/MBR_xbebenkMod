	...
	Switch $RoutineType
		Case "FCR0" ; FirstCheckRoutine
			...
			Else
				...
				'CollectAchievements', 'SaleMagicItem', 'CollectFreeMagicItems']
				_ArrayShuffle($aRndFuncList)
				For $Index In $aRndFuncList
					If Not $g_bRunState Then Return
					_RunFunction($Index)
			...
			Case "QKR1" ; QuickRoutine before 2nd attack
				Local $aRndFuncList = ['PetHouse', 'Collect', 'FstReq', 'ReplayShare', 'ForgeClanCapitalGold', 'CheckLeague', 'CollectAchievements', 'CollectCCGold']
				_ArrayShuffle($aRndFuncList)
				For $Index In $aRndFuncList
					If Not $g_bRunState Then Return
					_RunFunction($Index)
		...
		Case "SA3" ; switch
			Local $aRndFuncList = ['PetHouse', 'BuilderBase', 'Laboratory', 'CollectCCGold', 'AutoUpgradeCC', 'DonateCC,Train', 'UpgradeHeroes', 'UpgradeWall'] ;, 'UpgradeLow']
			For $Index In $aRndFuncList
				If Not $g_bRunState Then Return
				_RunFunction($Index)
				If _Sleep(100) Then Return
		...
		Case "NCGE2" ; NoClanGamesEvent
			Local $aRndFuncList = ['Collect', 'PetHouse', 'Laboratory', 'UpgradeHeroes', 'CollectCCGold', _
            'UpgradeLow', 'CollectCCGold', 'AutoUpgradeCC', 'BBRTN0']
			_ArrayShuffle($aRndFuncList)
			For $Index In $aRndFuncList
				If Not $g_bRunState Then Return
				_RunFunction($Index)
