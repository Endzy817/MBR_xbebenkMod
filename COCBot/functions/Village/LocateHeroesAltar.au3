...
Func _LocateQueenAltar($bFromButton = False)
	...
	While 1
		...
		$MsgBox = _ExtMsgBox(0, GetTranslatedFileIni("MBR Popups", "Ok_Cancel", "Ok|Cancel"), GetTranslatedFileIni("MBR Popups", "Func_Locate_Queen_Altar_02", "Locate Queen Altar at ") & $g_sAndroidTitle, $sText, 10)
		If $MsgBox = 1 Then
			WinGetAndroidHandle()
			ClickAway()
			Local $aPos = FindPos()
		...
		For $iCountGetInfo = 0 To 10
			...
			$sInfo = BuildingInfo(242, 490 + $g_iBottomOffsetYNew) ; Get building name and level with OCR
...
Func _LocateKingAltar($bFromButton = False)
	...
	While 1
		...
		$MsgBox = _ExtMsgBox(0, GetTranslatedFileIni("MBR Popups", "Ok_Cancel", "Ok|Cancel"), GetTranslatedFileIni("MBR Popups", "Func_Locate_King_Altar_02", "Locate King Altar at ") & $g_sAndroidTitle, $sText, 10)
		If $MsgBox = 1 Then
			WinGetAndroidHandle()
			Local $aPos = FindPos()
...
Func _LocateWardenAltar($bFromButton = False)
	...
	While 1
		...
		$MsgBox = _ExtMsgBox(0, GetTranslatedFileIni("MBR Popups", "Ok_Cancel", "Ok|Cancel"), GetTranslatedFileIni("MBR Popups", "Func_Locate_Warden_Altar_02", "Locate Grand Warden Altar at ") & $g_sAndroidTitle, $sText, 10)
		If $MsgBox = 1 Then
			WinGetAndroidHandle()
			Local $aPos = FindPos()
...
Func _LocateChampionAltar($bFromButton = False)
	...
	While 1
		...
		$MsgBox = _ExtMsgBox(0, GetTranslatedFileIni("MBR Popups", "Ok_Cancel", "Ok|Cancel"), GetTranslatedFileIni("MBR Popups", "Func_Locate_Champion_Altar_02", "Locate Royal Champion Altar at ") & $g_sAndroidTitle, $sText, 10)
		If $MsgBox = 1 Then
			WinGetAndroidHandle()
			ClickAway()
			Local $aPos = FindPos()
	...
	If $sTemplateDir <> "" Then
		For $i = 0 To 1
			If QuickMIS("BC1", $sTemplateDir, 100, 55, 775, 580 + 44, True, False) Then
				...
				For $iCountGetInfo = 0 To 10
					If _Sleep(500) Then Return
					$sInfo = BuildingInfo(260, 494) ;(242, 490 + $g_iBottomOffsetYNew) ; Get building name and level with OCR
					If IsArray($sInfo) Then ExitLoop
				Next
